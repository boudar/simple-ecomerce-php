...
?>
        <div id="page-content-wrapper">
            ...
            <div class="p-3 bg-dark text-white shadow-sm d-flex justify-content-around align-items-center rounded">
                            <div>
                            <?php
                                  $stmt1 = $con->prepare("SELECT COUNT(item_id) FROM items");
                                  $stmt1 -> execute();
                                  $result2 = $stmt1->fetch();
                                //   print_r($result2);
                                echo "<h3 class='fs-2'>".$result2['COUNT(item_id)']."</h3>";
                                ?>
                        ...
                        <table class="table table-dark table-hover" id="myTable">
                            ...
                            <tbody>
                                    ...
                                    foreach ($result as $info) {
                                        $id= $info['item_id'];
                                        echo "<tr>";
                                        echo "<td><img class='pimage' src='uploads/".$info['image']."'></td>";
                                        echo "<td>".$info['price']."DH</td>";
                                        echo "<td>".$info['title']."</td>";
                                        echo "<td>".$info['description']."</td>";
                                        echo "<td>".$info['category']."</td>";
                                        echo "<td><a id='texto' onclick='return confirm(\"are you sure you want to delete this user? \")' class='deluser' href='prodelete.php?productid=$id'>delete</a> 
                                        <a class='updauser'  href='proupdate.php?productid=$id'>update</a></td>";
                                        echo "<tr>";
                                    }
                                    ?>
                            </tbody>
